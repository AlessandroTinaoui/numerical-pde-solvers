...
void DiffusionReactionDN::setup()
{
  GridIn<dim> grid_in;
...
void DiffusionReactionDN::assemble()
{
...
void DiffusionReactionDN::solve()
{
...
void DiffusionReactionDN::output(const unsigned int iteration) const
{
  DataOut<dim> data_out;
  data_out.add_data_vector(dof_handler, solution, "solution");
  data_out.build_patches();

  const std::string file_name = "output-dn-" + std::to_string(subdomain_id) + "-" + std::to_string(iteration) + ".vtk";
  std::ofstream     output_file(file_name);
  data_out.write_vtk(output_file);
}

void DiffusionReactionDN::apply_interface_dirichlet(const DiffusionReactionDN &other)
{
...
double DiffusionReactionDN::apply_interface_neumann(DiffusionReactionDN &other, const double lambda)
{
  const auto           interface_map      = compute_interface_map(other);
...
const Vector<double> &DiffusionReactionDN::get_solution() const
{
  return solution;
}

void DiffusionReactionDN::apply_relaxation(const Vector<double> &old_solution, const double lambda)
{
  solution *= lambda;
  solution.add(1.0 - lambda, old_solution);
}

Vector<double> DiffusionReactionDN::compute_interface_residual()
{
  assemble();
...
std::map<types::global_dof_index, types::global_dof_index> DiffusionReactionDN::compute_interface_map(const DiffusionReactionDN &other) const
{
