...
void
DiffusionReactionDD::output(const unsigned int iteration) const
{
  DataOut<dim> data_out;
  data_out.add_data_vector(dof_handler, solution, "solution");
  data_out.build_patches();

  const std::string file_name = "output-dd-" + std::to_string(subdomain_id) +
...
void
DiffusionReactionDD::apply_interface_dirichlet(
  const DiffusionReactionDD &other)
{
...
double
DiffusionReactionDD::apply_interface_correction(DiffusionReactionDD &other,
                                                const double lambda,
                                                const double smoothing)
{
  ...
  FEFaceValues<dim> fe_face_values(*fe,
                                   quadrature_interface,
                                   update_values | update_gradients |
                                     update_normal_vectors | update_JxW_values);

  FullMatrix<double> interface_matrix(interface_dofs.size(),
                                      interface_dofs.size());
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    if (cell->at_boundary())
      for (unsigned int face = 0; face < cell->n_faces(); ++face)
        if (cell->face(face)->at_boundary() &&
            cell->face(face)->boundary_id() == interface_id)
          {
            fe_face_values.reinit(cell, face);
            cell->get_dof_indices(dof_indices);
            ...
            for (unsigned int q = 0; q < quadrature_interface.size(); ++q)
              for (unsigned int i = 0; i < fe->dofs_per_cell; ++i)
                for (unsigned int j = 0; j < fe->dofs_per_cell; ++j)
                  if (interface_indices.count(dof_indices[i]) > 0 &&
                      interface_indices.count(dof_indices[j]) > 0)
                    {
                      ...
                      const double tangential_product =
                        grad_i * grad_j - (grad_i * normal) * (grad_j * normal);

                      interface_matrix(interface_indices.at(dof_indices[i]),
                                       interface_indices.at(dof_indices[j])) +=
                        (fe_face_values.shape_value(i, q) *
                           fe_face_values.shape_value(j, q) +
                         smoothing * tangential_product) *
  ...
  for (const auto &[dof, other_dof] : interface_map)
    flux_imbalance[interface_indices.at(dof)] =
      residual[dof] + other_residual[other_dof];
...
Vector<double>
DiffusionReactionDD::compute_interface_residual()
{
  assemble();
...
DiffusionReactionDD::compute_interface_map(
  const DiffusionReactionDD &other) const
{
