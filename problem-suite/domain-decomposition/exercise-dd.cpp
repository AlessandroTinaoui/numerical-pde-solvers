...
int main(int argc, char *argv[])
{
  ...
  const auto alpha = [](const Point<DiffusionReactionDD::dim> &) { return 1.0; };
  const auto gamma = [](const Point<DiffusionReactionDD::dim> &) { return 1.0; };
  const auto f     = [](const Point<DiffusionReactionDD::dim> &) { return 1.0; };
  ...
  while (n_iter < n_max_iter && correction_norm > tolerance_correction)
    {
      ...
      std::cout << "iteration " << n_iter << " - interface correction = " << correction_norm << std::endl;
