...
Poisson2D::setup()
{
  // Create the mesh.
  {
    ...
    std::ifstream grid_in_file("../mesh/mesh-problem-" + std::to_string(subdomain_id) + ".msh");
...
Poisson2D::output(const unsigned int &iter) const
{
  ...
  const std::string output_file_name = "output-" + std::to_string(subdomain_id) + "-" + std::to_string(iter) + ".vtk";
...
Poisson2D::compute_interface_map(const Poisson2D &other) const
{
  ...
  if (subdomain_id == 0)
    {
      current_interface_dofs = DoFTools::extract_boundary_dofs(dof_handler, ComponentMask(), {1});
      other_interface_dofs = DoFTools::extract_boundary_dofs(other.dof_handler,ComponentMask(),{0});
    }
  else
    {
      current_interface_dofs = DoFTools::extract_boundary_dofs(dof_handler, ComponentMask(), {0});
      other_interface_dofs = DoFTools::extract_boundary_dofs(other.dof_handler,ComponentMask(),{1});
    }
  ...
  for (const auto &dof_current : current_interface_dofs)
    {
      ...
      for (const auto &dof_other : other_interface_dofs)
        {
          if (p.distance_square(other.support_points.at(dof_other)) < p.distance_square(other.support_points.at(nearest)))
            nearest = dof_other;
        }
