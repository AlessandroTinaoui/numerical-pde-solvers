...
public:
  ...
  using TractionFunction    = std::function<Tensor<1, dim>(const Point<dim> &, const Tensor<1, dim> &)>;
  using DirichletConditions = std::map<types::boundary_id, const Function<dim> *>;
  using TractionConditions  = std::map<types::boundary_id, TractionFunction>;
